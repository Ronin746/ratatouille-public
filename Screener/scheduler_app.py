...
# ── Intermediate data directory for multi-market merge ─────────────────────
_HERE = os.path.dirname(os.path.abspath(__file__))
_DATA_DIR = os.path.join(os.path.dirname(_HERE), 'Data')
_INTERMEDIATE_DIR = os.path.join(_DATA_DIR, 'intermediate')


def save_intermediate_scores(df: pd.DataFrame, region: str):
    """Save scored DataFrame as parquet for later merge."""
    os.makedirs(_INTERMEDIATE_DIR, exist_ok=True)
    path = os.path.join(_INTERMEDIATE_DIR, f"{region}_scores.parquet")
    df.to_parquet(path, engine="pyarrow")
    logger.info("Intermediate scores saved: %s (%d rows)", path, len(df))


def load_intermediate_scores(region: str) -> pd.DataFrame | None:
    """Load intermediate scores if available and from today's session."""
    path = os.path.join(_INTERMEDIATE_DIR, f"{region}_scores.parquet")
    if not os.path.exists(path):
        logger.warning("No intermediate file for region '%s'.", region)
        return None
    try:
        df = pd.read_parquet(path, engine="pyarrow")
        logger.info("Loaded intermediate %s scores: %d rows.", region, len(df))
        return df
    except Exception as e:
        logger.error("Failed to load intermediate %s: %s", region, e)
        return None


def deduplicate_by_volume(df: pd.DataFrame) -> pd.DataFrame:
    """
    Deduplicate cross-listed tickers: keep only the primary listing
    (highest avg_dollar_vol) for each base symbol.
    E.g., SAP.DE vs SAP.MU → keep the one with higher volume.
    """
    if df.empty:
        return df

    # Extract base symbol by stripping exchange suffix
    idx = df.index.astype(str)
    base_symbols = idx.map(lambda t: t.split('.')[0])
    df = df.copy()
    df['_base'] = base_symbols
    df['_vol'] = df.get('avg_dollar_vol', pd.Series(0, index=df.index))

    # For each base symbol, keep the row with the highest volume
    keep_idx = df.groupby('_base')['_vol'].idxmax()
    deduped = df.loc[keep_idx].drop(columns=['_base', '_vol'])

    removed = len(df) - len(deduped)
    if removed > 0:
        logger.info("Deduplication: removed %d cross-listed duplicates (%d → %d).",
                    removed, len(df), len(deduped))
    return deduped
    ...
    for ticker in tickers:
        try:
            ...
            row = {
                'Ticker': ticker,
                # Price Performance
                '6m_return':  pp.get('6m_return', 0.0),
